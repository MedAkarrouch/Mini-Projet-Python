...
import { getColumnsFromFile } from "../../../utils/fileutils"
const LoadFile = () => {
  ...
  const handleCsvChange = async (e) => {
    ...
    if (!file) return
    try {
      const columns = await getColumnsFromFile(file)
  ...
  return (
    ...
    <div className="px-6 py-12 flex flex-col justify-between min-h-[inherit]">
      ...
      {file && (task === "classification" || task === "regression") && (
        <div>
          <label
            htmlFor="first_name"
            className="block mb-4 text-sm font-medium text-gray-900"
      ...
      <div className="mt-4 flex justify-end space-x-4 text-base font-semibold">
        ...
        <button
          disabled={!file || !datasetTitle || isAdding}
